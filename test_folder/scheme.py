...
def check_formals(formals):
    ...
    while isinstance(formals, Pair) and formals != nil:
        check_sys_helper(formals.first)
...
def scheme_optimized_eval(expr, env):
    ...
    while True:
        ...
        if first in LOGIC_FORMS:
            ...
            "*** YOUR CODE HERE ***"
            opt_procedure = scheme_optimized_eval(first, env)
            args = rest.map(lambda operand: scheme_optimized_eval(operand, env))
            if isinstance(opt_procedure, LambdaProcedure):
                expr = opt_procedure.body
                env = opt_procedure.env.make_call_frame(opt_procedure.formals, args)
            elif isinstance(opt_procedure, MuProcedure):
                expr = opt_procedure.body
                env = env.make_call_frame(opt_procedure.formals, args)
            elif isinstance(opt_procedure, PrimitiveProcedure):
                return scheme_apply(opt_procedure, args, env)
            else:
                raise SchemeError("not alble to call {0}".format(str(opt_procedure)))
